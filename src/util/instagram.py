...
class Instagram():
    ...
    def login(self):
        try:
            self.log_obj.debug("Testing in-progress for Instagram")
            # Open login page
            ...
            time.sleep(5)
            for result_pattern in self.config['result_patterns']['failure']:
                if result_pattern in self.browser.browser.page_source:
